# NSFW Chatbots

**Open source AI NSFW directory** of adult chatbots — the first and most comprehensive directory of NSFW AI chat characters on the internet.

This platform is an open gateway to the directory for anyone looking for a specific kind of character to flirt with and practice their specific kink or fetish.

---

## Getting Started

To start browsing for the best NSFW chatbots, simply follow these steps:

1. Browse through our selection of categories and choose the one that best fits your interests.
2. Once there, click on the **Chat** button below the chatbot's profile picture to access their private page, where you can find more information about them.
3. Create an account on the platform hosting the respective character and start enjoying your NSFW AI chat with your new virtual companion.

---

## Links

* [Contact](https://nsfwchatbots.net/contact/)
* [Terms of Service](https://nsfwchatbots.net/terms-of-service/))
* [Privacy Policy](https://nsfwchatbots.net/privacy-policy/)
* [Blog](https://nsfwchatbots.net/blog/)

## Acknowledgments

* [CharacterAIAlternative](https://characteraialternative.blogspot.com) - Featured as a Character.ai alternative
---
